...
INPUT_SIZE_MB=$(awk -v b="${INPUT_SIZE_BYTES}" 'BEGIN{printf "%.3f", b/1024/1024}')
echo "[WSL] 测试文件: ${INPUT_FILE} (${INPUT_SIZE_MB} MB)" | tee -a "${LOG_DIR}/wsl_server_upload.log"
...
UPLOAD_STATUS=$?
UPLOAD_END_MS=$(get_time_ms)
UPLOAD_ELAPSED_MS=$((UPLOAD_END_MS - UPLOAD_START_MS))
[[ ${UPLOAD_ELAPSED_MS} -le 0 ]] && UPLOAD_ELAPSED_MS=1

UPLOAD_THROUGHPUT_MB_S=$(awk -v mb="${INPUT_SIZE_MB}" -v ms="${UPLOAD_ELAPSED_MS}" 'BEGIN{printf "%.3f", (mb*1000)/ms}')

if [[ ${UPLOAD_STATUS} -eq 0 ]]; then
  echo "[WSL] ✓ 上传成功" | tee -a "${LOG_DIR}/wsl_server_upload.log"
  echo "[WSL] 上传耗时: ${UPLOAD_ELAPSED_MS} ms" | tee -a "${LOG_DIR}/wsl_server_upload.log"
  echo "[WSL] 等效上传吞吐: ${UPLOAD_THROUGHPUT_MB_S} MB/s" | tee -a "${LOG_DIR}/wsl_server_upload.log"
